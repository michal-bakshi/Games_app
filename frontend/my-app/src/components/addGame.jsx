...
export const AddGame=()=>{
    ...
    return<form className="container" onSubmit={(e)=>handleGame(e)}>
        <h2 className="">הוספת משחק</h2>
        <p className="text-danger ">שדות המסומנים בכוכבית הם שדות חובה</p>
    <div className="col">
        <input className="form-control row m-2" type="text" placeholder=" *הכנס שם משחק" 
        onBlur={(x)=>setObjGame({...objGame,name:x.target.value})} required={isForUpdate === ""}/>
     ...
     {/* לבדוק שיש קוד כזה */}
        {/* <input className="form-control row m-2" type="text" placeholder="*הכנס קוד קטגוריה" 
         ...
         <label>הכנס שם קטגוריה</label>
          <select className="m-2 form-select" onBlur={(x)=>setObjGame({...objGame,code_Category:x.target.value})}required={isForUpdate === ""} > 
           {kat.map((x)=>
    ...
    <div className="col">
        <input className="form-control row m-2" type="number" placeholder="*הכנס מחיר משחק " 
        onBlur={(x)=>setObjGame({...objGame,price:x.target.value})} required={isForUpdate === ""}/>
     </div>
  ...
  >
    ...
    <option value="נחשמי.jpg">נחשמי</option>
     ...
     <div className="col">
        <input className="form-control row m-2" type="number" placeholder="*כמות במלאי" 
        onBlur={(x)=>setObjGame({...objGame,amount:x.target.value})} required={isForUpdate === ""}/>
     </div>
     <input type="submit" className="btn btn-primary"  value={isForUpdate!== "" ? "עדכן" : "הוסף"}/>
