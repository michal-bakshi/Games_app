...
export const AddKategoty=()=>{
    ...
    // const checkCode=(e)=>{
    //     const codeK=e.target.value
    //     if(codeK=="")
    //         setException({...exception,codeK:"שדה זה הוא שדה חובה"})
    //     else if(!codeK.match("[1-9]"))
    //         setException({...exception,codeK:"הכנס רק מספרים"})
    //     else{
    //         setKat({...kat,id:codeK})
    //         setException({...exception,codeK:""})
    //     }
    // }
    ...
    return<form className="container" onSubmit={(e)=>handleCaterogy(e)}>
        <h1>הוספת קטגוריה</h1>
        <div className="col">
            <input className="form-control row m-2" type="text" value={kat.name} placeholder="הכנס שם קטגוריה" 
