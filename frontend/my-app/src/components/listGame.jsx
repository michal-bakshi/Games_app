  ...
  return (
    <div className="container m-3">
      ...
      <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
        {list.length === 0 && <p> אין משחקים בקטגוריה עדיין...</p>}
        {list.map((x, index) => (
            ...
            <div className="card h-100 shadow-sm">
              <div className="d-flex justify-content-center">
                <img
                  ...
                  src={`http://localhost:9090/${x.pic}`}
                  className="card-img-top "
                  alt={`Image for ${x.name}`}
                />
              </div>
              <div className="card-body">
                ...
                <button onClick={() => moreDetails(x)} className="m-1 btn btn-primary">
                  פרטים נוספים
                </button>
                {isManeger && (
                  <button onClick={() => deleteGameFunc(x._id)} className="m-1 btn btn-danger">
                    מחיקה
                  </button>
                )}
                {isManeger && (
                  <button onClick={() => updateGameFunc(x._id)} className="m-1 btn btn-primary">
                    עדכון
                  </button>
                )}
              </div>
